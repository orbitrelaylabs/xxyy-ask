...
| Skill                           | 应用内实现                                     | 外部接入                               | 数据来源                   |
| ------------------------------- | ---------------------------------------------- | -------------------------------------- | -------------------------- |
| `xxyy-product-support`          | `product-support-runtime` 直接调用 Product RAG | `scripts/ask.mjs` 调用受保护 Chat API  | 正式产品知识库             |
...
- 产品问答在同一进程直接调用检索 runtime；外部 Agent 复用受保护 HTTP API。
