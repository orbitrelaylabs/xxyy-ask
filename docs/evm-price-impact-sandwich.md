...
该包没有 RPC、Indexer、Explorer、环境变量、LLM、LangGraph、Capability 或 MCP 依赖。通用 `onchain-analysis` MCP 可把已验证 observation 交给该纯函数 core；`.env.example` 中的免费公共 RPC 便利配置不会声称具备这种 evidence。XXYY Web/API/Telegram 已通过精确 grant 注册 `detect_sandwich`，但只消费该 core 的有界四态投影；缺 archive/pool/actor coverage 时必须 abstain，不能把缺证据当作未被夹。
