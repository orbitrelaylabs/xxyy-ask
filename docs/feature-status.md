...
- [x] Telegram Bot：`pnpm run telegram:dev` 通过 Telegram Bot API long polling 接收消息；私聊文本直接回答。群聊默认只读，不发送命令回复、客服答案、媒体或异常。群文本实时幂等写入本地 PostgreSQL 审计缓冲并入持久化整理队列；原始消息默认保留 30 天。Web 不采集客服对话用于知识演进。
- [x] Telegram 群注册表与自动整理：Bot 订阅 `my_chat_member` 并登记群元数据，同时保存 Telegram 已交付的群文本、Reply 关系、作者 ID、编辑和处理状态。独立 `telegram-curation-worker` 合并同群连续消息、租约领取任务并失败重试；“Telegram 群聊”页面只显示任务状态和待识别数量，不展示原始消息或人工整理按钮。
- [x] Knowledge Curator Auto Mode：自动整理 Worker 会合并连续发言、识别 Reply 与紧邻管理员普通回答，并按角色有效期验证作者、脱敏、分类、标准化、去重、检查正式 chunk 冲突并生成质量/风险信息。
- [x] 候选 AI 优化建议：Pending 候选的“标准答案”输入框下提供按需优化按钮，使用尚未保存的当前问题/答案生成证据受控的答案、缺失信息和风险建议；服务端拒绝新增无证据数字或 URL。建议不自动写库、审批或发布，应用后仍需保存不可变 Revision。
- [x] 分来源知识治理：官方同步和遗留自动来源继续由 `knowledge-automation-v1` 执行严格自动决策；本地 Telegram 候选固定带 `manual_review_required`，自动整理不会批准或拒绝，必须由后台管理员审核。批准后自动创建发布任务，模型不能决定发布。
...
- [x] Browser-only XXYY Onchain MCP / Skill：Web、Telegram 和 `pnpm onchain:mcp:dev` 只通过隔离 Chrome Profile 访问固定 Explorer 与 XXYY 页面，支持 Solana、Ethereum、BSC、Base、Robinhood Chain 和 Stable Chain 的公开交易引用。交易事实始终标为单源 partial；Sandwich 只输出结构性模式或数据不足，池子正确性需要显式 canonical 声明，精确成交会生成面向用户的标注截图。
...
- [x] 本地开发命令：启动入口统一为 `pnpm run app:dev`、`pnpm run api:dev`、`pnpm run web:dev`、`pnpm run telegram:dev` 和 `pnpm telegram:curation:worker`；启动前更新可用 `app:dev` 的 `--sync`、`--full-sync` 或 `--ingest`，独立调度使用 `pnpm rag:refresh`。
