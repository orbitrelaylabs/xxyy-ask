...
- [x] 进程内 nested tracing：vendor-neutral tracer 覆盖 planner、tool、retrieval、rerank、grounding 和 answer，只用于 provider-backed 评测且不向外部平台上传。
...
5. **Goal 24 / Generic Onchain MCP-Skill Capability**：仓库实现已提前完成并解耦产品命名：`chain.get_transaction`、`chain.inspect_transaction` 与 `chain.detect_sandwich` 已封装为通用只读 MCP/Skills，具备显式授权、超时、成本、审计与结构化错误。通用 MCP 已通过必填环境配置覆盖 XXYY 当前六条产品链，`.env.example` 提供公共 RPC 便利值但运行时代码不内置 endpoint；XXYY 深度生产 composition 仍必须等 20B-4 返回未过期 `ready` 并通过 readiness/manifest lineage 门禁，免费公共 RPC 或现有 Product MCP/Skill 都不构成链上 readiness 证据。
