...
XXYY 浏览器兼容层使用链上提取的完整交易钱包地址和交易时间调用页面原生历史筛选，从前后 2 秒开始逐级扩大到 15 秒、120 秒；Explorer URL 会先归一化为裸交易哈希，再对钱包与时间命中的行执行完整哈希和池地址校验。流程在同一个后台 Target 中选择仍包含目标完整哈希的最大上下文，避免高频池的 50 条显示上限把目标挤出结果。页面组件提供的 `blockNumber` 与 `logIndex` 会保留到目标和相邻成交证据中；EVM 周边成交已有这些字段时不再为每一行重复打开 Explorer。
