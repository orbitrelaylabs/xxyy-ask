    ...
    messages: [
      {
        ...
        content: [
          ...
          '{"kind":"tool","route":"chain_answer","toolName":"get_public_transaction","input":{"query":"..."},"reason":"..."}',
          '{"kind":"tool","route":"chain_answer","toolName":"diagnose_xxyy_transaction","input":{"reference":"...","network":"...","checks":["sandwich","pool"]},"reason":"..."}',
          ...
          'Valid route values are exactly: agent_answer, boundary, chain_answer, clarify, product_answer, unsupported.',
          ...
          '- Read-only analysis of a supplied public transaction is available only when get_public_transaction appears in the tool list.',
          '- Use get_public_transaction only for a supported public Explorer transaction URL, or an explicit network plus transaction id. It returns browser-visible status, block or slot, timestamp, addresses, value, fee, and token-transfer facts. It does not provide call traces or MEV conclusions.',
          '- When diagnose_xxyy_transaction appears in the tool list, use it for exactly one supplied public transaction when the user asks whether an XXYY trade was sandwiched, used the wrong pool, or used a small-liquidity pool. Select only the requested checks and preserve the supplied Explorer URL as reference; a bare hash requires network.',
          '- XXYY Sandwich diagnosis uses the target and surrounding XXYY trade structure. Preserve confirmed, likely, unlikely, and insufficient_data exactly; never turn missing browser or market evidence into a positive or negative conclusion.',
          '- Wallet/account lookup, arbitrary address history, private data, signing, simulation, and trading operations are not available in this runtime.',
