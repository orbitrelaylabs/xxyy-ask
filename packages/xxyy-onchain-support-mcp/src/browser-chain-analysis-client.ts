...
export function createBrowserChainAnalysisClient(
  ...
  return {
    ...
    async detectSandwich() {
      throw new Error('Browser evidence does not provide verified EVM MEV observations.');
    },
    ...
    async inspectTransaction() {
      throw new Error('Browser evidence does not provide verified EVM execution traces.');
    },
  };
}
...
async function loadScanPageTransaction(input: {
  ...
  const expression = `(() => {
    ...
    const timestampText = text.match(/(?:Timestamp|时间戳)[^\\n]*\\n?([^\\n]+)/i)?.[1] || '';
    const unix = timestampText.match(/\\((\\d{10})\\)/)?.[1];
    ...
    if (!hash || !block || addressLinks.length === 0) return null;
    return {
      accountAddresses:[...new Set(addressLinks)], blockNumber:block, feeWei:toWei(feeEth),
      from:addressLinks[0], hash, rawInput:'0x',
      status:/success|成功/i.test(statusText)?'success':/fail|error|失败/i.test(statusText)?'reverted':'unknown',
      timestamp:unix ? new Date(Number(unix)*1000).toISOString() : new Date().toISOString(),
      to:addressLinks[1] || null, tokenAddresses:[...new Set(tokenLinks)], tokenTransfers:[], valueWei:toWei(valueEth)
    };
  })()`;
  const raw = await evaluateBrowserPage({ ...input, expression, url: explorerUrl });
